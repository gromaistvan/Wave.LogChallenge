...
internal sealed class SyncSuperFastLogReader(string fileName, int bufferSize = 1_000_000): IAsyncLogReader
{
    ...
    private (long lines, string firstName, string secondName, string eachMonth, string commonName) Read(IProgress<decimal> progress)
    {
    ...
    public Task<(long lines, string firstName, string secondName, string eachMonth, string commonName)> ReadAsync(IProgress<decimal> progress, CancellationToken token = default) => Task.Run(() => Read(progress), token);
}
